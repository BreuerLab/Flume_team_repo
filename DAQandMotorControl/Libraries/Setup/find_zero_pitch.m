function [last_out] = find_zero_pitch(dq,last_out,Wbias,Gbias,accbias,pitch_bias)
% Finds the minimum lift on wallace hydrofoil while under flow.

disp('finding zero.  Wallace will now move +/- 5 degrees')
move_new_pos_3rigs(dq,last_out,[0 0 0 0 5 0],2,Wbias,Gbias,accbias,pitch_bias);
...
if round(f(2)) < 0 || round(f(2)) > numel(prof(:,5))
    ...
    move_new_pos_3rigs(dq,last_out,[0 0 0 0 15 0],2,Wbias,Gbias,accbias,pitch_bias);
