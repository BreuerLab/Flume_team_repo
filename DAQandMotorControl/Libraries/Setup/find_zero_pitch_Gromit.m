function [last_out, pitch_bias] = find_zero_pitch_Gromit(dq,last_out,Wbias,Gbias,accbias,pitch_bias)
...
[out,prof,last_out] = move_new_pos_3rigs(dq,last_out,[0 0 5 0 0 0],2,Wbias,Gbias,accbias,pitch_bias); % added output to get "last_out" for the next move_new_pos_3rigs
[out,prof,last_out] = move_new_pos_3rigs(dq,last_out,[0 0 -5 0 0 0],10,Wbias,Gbias,accbias,pitch_bias);
...
%Lift = (1:numel(Lift)-b)./a)
 [out,prof2,last_out] =  move_new_pos_3rigs(dq,last_out,[0 0 5 0 0 0],10,Wbias,Gbias,accbias,pitch_bias);
...
if round(f(2)) < 0 || round(f(2)) > numel(prof(:,3))
 ...
 [out,prof,last_out] = move_new_pos_3rigs(dq,last_out,[0 0 15 0 0 0],2,Wbias,Gbias,accbias,pitch_bias);
 [out,prof,last_out] = move_new_pos_3rigs(dq,last_out,[0 0 -15 0 0 0],10,Wbias,Gbias,accbias,pitch_bias);
    ...
    plot(1:numel(prof(:,1)),prof(:,3),1:numel(out(:,18)),smooth(Lift,100),1:numel(Lift),((1:numel(Lift))-f(2))./f(1))
    hold on
 [out,prof2,last_out] =  move_new_pos_3rigs(dq,last_out,[0 0 15 0 0 0],10,Wbias,Gbias,accbias,pitch_bias);
    ...
    pitch_bias(2) = mean([prof(round(f(2)),3) prof2(round(f1(2)),3)]);
    ...
    [out,prof,last_out] = move_new_pos_3rigs(dq,last_out,[0 0 5 0 0 0],2,Wbias,Gbias,accbias,pitch_bias);
    [out,prof,last_out] = move_new_pos_3rigs(dq,last_out,[0 0 -5 0 0 0],10,Wbias,Gbias,accbias,pitch_bias);
...
pitch_bias(2) = mean([prof(round(f(2)),3) prof2(round(f1(2)),3)]);          % Based on zero lift
