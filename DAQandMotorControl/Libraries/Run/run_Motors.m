function [flume, out, dat, Prof_out_angle, Prof_out,last_out, freq,pitch2, heave2, pitch3, heave3,phase13, num_cyc, phi, foiltype]...
    ...
    num_cyc, transientcycs, constantamplitude, betah, betap, offset)
%%
...
Prof_out_temp = input_conv_3rigs(Prof_out_angle, freq, heave1, heave2, heave3, bias.pitch); % changed to account for PIV

% For PIV trigger (always on, probably we can ignore it)

trig_signal = ones(length(Prof_out_temp(:,1)),1);
trig_signal(1:round(1/freq*dq.Rate*offset)) = 0;
trig_signal(end-round(1/freq*dq.Rate)*offset:end) = 0;
% 
Prof_out = [Prof_out_temp trig_signal];

% Prof_out = Prof_out_temp;
