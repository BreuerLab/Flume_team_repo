...
foldername = sprintf('%s_SingleMonday_SingleFoil_A3E_Fred', datetime('now','Format','yyyyMMdd')); % name of the new directory
...
betah = 0; % for trapezoidal/triangular motion
betap = 0;
formatOut = 'yyyymmdd';
foiltype = 'A3E';
...
[flume, out, dat, Prof_out_angle, Prof_out, last_out, freq, pitch2, heave2, pitch3, heave3, phase13, num_cyc, phi, foiltype]...
        ...
        foiltype, 0.9, 0, 0, 70, 0.8, 0, -90, 10, 3, 0, betah, betap); % check if phi is positive or negative
% foiltype, freq, pitch2, heave2, pitch3, heave3, phase13, phi, num_cyc, transientcycs, constantamplitude, betah, betap);
...
P3 = 70; % Wallace, in degrees
H3 = 0.8;  % Wallace, in chords (non-dimensional)

phase13  = 0;  % phase between both rigs
phi      = -90;   % phase between heaving and pitching

num_cyc  = 40;   % number of full-amplitude experimental cycles
...
for j = 1:length(freq)
    ...
    [flume, out, dat, Prof_out_angle, Prof_out, last_out, freqj, pitch2, heave2, pitch3, heave3, phase13, num_cyc, phi, foiltype]...
        = run_Motors(dq,last_out,pitch_bias,Wbias,Gbias,accbias,...
        foiltype, freq(j), P2, H2, P3, H3, phase13, phi, num_cyc, transientcycs, constantamplitude, betah, betap);
    
    alphaT4 = atan(-2*pi*heave3*fred(j)) + deg2rad(pitch3); % self-explanatory

    file = [datestr(now,formatOut), '_SingleFoil_Fred_sweep', foiltype, '_f=', num2str(fred(j),3), '.mat'];
    filename = fullfile(folderpath, file);
    
%     save(filename, 'dat', 'Prof_out_angle', 'out', 'foiltype', 'fred', 'freqj', 'Re', 'alphaT4', 'num_cyc');
    save(filename);
